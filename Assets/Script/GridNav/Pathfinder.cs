...
namespace Astar
{
    ...
    public class Pathfinder
    {
        ...
        {
            return Solve(g, g.TranslateCoordinate(origin), g.TranslateCoordinate(dest), Mathf.FloorToInt(range / g.UnitSize) - 2);
        }
        public static PathfinderPath SolvePath(GridNav g, GridNav.Node origin, GridNav.Node dest)
        {
            return Solve(g, origin.gridPos, dest.gridPos, 0);
        }
        public static PathfinderPath SolvePath(GridNav g, Vector2Int origin, Vector2Int dest)
        {
            return Solve(g, origin, dest, 0);
        }
        ...
        public void solve()
        {
            ...
            currentCell = new Node(grid.GetNodeAt(origin), Node.CalcDistance(origin, dest), 0);
        ...
        private void stepPathfinder()
        {
            ...
            foreach (GridNav.Node n in currentCell.point.neighbors)
            {
                if (n != null && n.passible)
                {
                    ...
                    if (!exists)
                    {
                        openList.Add(new(n, Node.CalcDistance(dest, n.gridPos), currentCell.index + 1));
                    }
                }
            }
